...
class AbstractEtcd(AbstractDCS):
    ...
    def set_site_ttl(self, site_ttl: int):
...
class Etcd(AbstractEtcd):
    ...
    def _cluster_from_nodes(self, etcd_index: int, nodes: Dict[str, etcd.EtcdResult]) -> Cluster:
        ...
        return Cluster(initialize, config, leader, status, members, failover, sync, history, failsafe, site)
    ...
    def touch_site(self, site: str) -> bool:
        return bool(self._client.set(self.site_path, site, self._site_ttl))

    @catch_etcd_errors
    def delete_site(self) -> bool:
        return bool(self._client.delete(self.site_path))
