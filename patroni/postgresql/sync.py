...
class _Replica(NamedTuple):
    ...
    :ivar site: the site tag of the replica
    """
    pid: int
        ...
        for row in postgresql.pg_stat_replication():
            ...
            if member and row[sort_col] is not None and member.is_running and not member.nosync:
                self.append(_Replica(row['pid'], row['application_name'],
                                     row['sync_state'], row[sort_col], bool(member.nofailover), member.site))
...
END;$$""")
    ...
    def current_state(self, cluster: Cluster) -> Tuple[CaseInsensitiveSet, CaseInsensitiveSet]:
        ...
        synchronous_across_sites_node_count = self._postgresql.global_config.synchronous_across_sites_node_count
        ...
        for replica in sorted(replica_list, key=lambda x: x.nofailover):
            if sync_node_maxlag <= 0 or replica_list.max_lsn - replica.lsn <= sync_node_maxlag:
                if cluster.site == replica.site:
                    same_site_candidates.add(replica.application_name)
                else:
                    other_site_candidates.add(replica.application_name)
                if replica.sync_state == 'sync' and replica.application_name in self._ready_replicas:
                    sync_nodes.add(replica.application_name)
            if len(same_site_candidates) >= sync_node_count:
                if len(other_site_candidates) >= synchronous_across_sites_node_count:
                    break
