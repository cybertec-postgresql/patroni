...
class Ha(object):
    ...
    def process_sync_replication(self) -> None:
        ...
        if self.is_synchronous_mode():
            ...
            additional = CaseInsensitiveSet(self.global_config.synchronous_nodes_additional)
            current_additional = CaseInsensitiveSet(sync.additional_members)
            ...
            if picked != current or additional != current_additional:
                ...
                if sync_common != current:
                    ...
                    sync = self.dcs.write_sync_state(self.state_handler.name, sync_common,
                ...
                if (allow_promote and allow_promote != sync_common) or (additional != current_additional):
                    if not self.dcs.write_sync_state(self.state_handler.name, allow_promote,
                                                     synchronous_nodes_additional=additional,
                                                     version=sync.version):
                        return logger.info("Synchronous replication key updated by someone else")
                    logger.info("Synchronous standby status assigned to %s", list(allow_promote) + list(additional))
        else:
    ...
    def enforce_primary_role(self, message: str, promote_message: str) -> str:
        ...
        if self.state_handler.is_primary():
            ...
            if self.is_synchronous_mode():
                ...
                if not self.dcs.write_sync_state(self.state_handler.name, None, None,
                                                 version=self.cluster.sync.version):
